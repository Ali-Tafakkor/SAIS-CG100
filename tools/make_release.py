"""Create the three firmware release assets from successful source builds."""
...
def make(tag: str, output: Path) -> dict:
    ...
    files = {}
    for name, source in (("g100-api.bin", app), ("stage0.bin", stage0)):
    ...
    admit(app.read_bytes(), FACTORY_BYTES - HEADER)
    ...
    match = re.search(r'#ifdef G100_SHADOW\s*#define G100_FIRMWARE_VERSION\s+"([^"]+)"', config)
    if not match:
        raise ValueError("Shadow firmware version is missing")
    version = match.group(1)
    commit = subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip()
    manifest = {"schema": 1, "release_tag": tag, "source_commit": commit,
